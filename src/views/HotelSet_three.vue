  ...
  <div class="p-container">
    <div class="container">
      ...
      <div class="content">
        ...
        <!-- 第一 -->
        <div class="flex-sta">
          <div class="item-b infoedit">
            <div>酒店名称：</div>
            <div  class="item-info">{{ hotelMsg.hotelName }}</div>
          </div>
          <div class="item-b">
            <div>酒店背景：</div>
            <div class="banner">
              ...
              <div class="upload upload-input" v-if="uploadUrl.length <= 5">
                <span>+</span>
                <input
                  ref="input"
                  type="file"
        ...
        <div class="flex-sta">
          ...
          <div class="item-b">
            <div>酒店管理员：</div>
            <div class="item-info"  style="width:280px">{{ hotelMsg.contactName }}</div>
          </div>
        </div>
        <!-- 第三 -->
        <div class="flex-sta">
          <div class="item-b infoedit">
            <div>联系方式：</div>
            <div class="item-info">{{ hotelMsg.contactPhone }}</div>
          </div>
          <div class="item-b">
            <div>可预订天数：</div>
            <div>
        ...
        <!-- 地址信息 -->
        <div class="item-b adress-box">
          <div>地址设置：</div>
           <!-- <div class="item-info-adress">{{address.province}}{{address.city}}{{address.area}}{{hotelMsg.hotelAddress}}</div> -->
 <div class="map-container-main">
    ...
    >
      <el-form-item prop="province">
        <el-input
          type="text"
          v-model="address.province" autocomplete="off"></el-input>
      </el-form-item>
      <el-form-item prop="city">
        <el-input
          type="text"
          v-model="address.city" autocomplete="off"></el-input>
      </el-form-item>
      <el-form-item prop="area">
        <el-input
          type="text"
          v-model="address.area" autocomplete="off"></el-input>
      </el-form-item>
       <el-form-item>
        <el-button type="warning" size="mini" @click="clicklocation()">定位</el-button>
      </el-form-item>
    </el-form>
     <el-input
          style="width: 365px;margin-bottom:20px;"
          type="text" size="mini" v-model="hotelMsg.hotelAddress" autocomplete="off"></el-input>
     <el-form
      size="mini"
      inline
    >
      <el-form-item label="经度：">
        <el-input
          type="text"
          v-model="hotelMsg.lng" autocomplete="off"></el-input>
      </el-form-item>
      <el-form-item label="纬度：">
        <el-input
          type="text"
          v-model="hotelMsg.lat"
          autocomplete="off"
        ></el-input>
      </el-form-item>
    </el-form>
    <div class="map-title">定位：</div>
    <div class="map_box">
        <vueMap @setlng="seting" @setlat="setlat"
        @setprovince="setprovince"
        @setcity="setcity"
        @setdistrict="setdistrict"
        @sethotelAddress="sethotelAddress"
        :lng="hotelMsg.lng" :lat="hotelMsg.lat" :mapsearchval="mapsearchval"/>
    </div>
  </div>
        </div>
        <div class="item-b">
...
export default {
  ...
  methods: {
    //收到地图传来的经纬度
    seting(val){
      this.hotelMsg.lng=val
    },
    setlat(val){
      this.hotelMsg.lat=val
    },
      setprovince(val){
      ...
      sethotelAddress(val){
      this.hotelMsg.hotelAddress=val
    },
    //todo:点击定位按钮
    clicklocation(){
        this.mapsearchval=`${this.address.province}${this.address.city}${this.address.area}${this.hotelMsg.hotelAddress}`
    },
    //点击复制授权码
    copy() {
    ...
    getAddress(arr, provinceCode, cityCode, countyCode) {
      ...
      arr.forEach((e) => {
        e.areaId == provinceCode && (address.province += e.areaName);
        if (cityCode && e.cities) {
          e.cities.forEach((e) => {
            e.areaId == cityCode && (address.city += e.areaName);
            if (countyCode && e.counties) {
              e.counties.forEach((e) => {
                e.areaId == countyCode && (address.area += e.areaName);
              });
            }
          });
        }
